...
class DiagnosisService:
    ...
    - RA3: Vive en la capa de aplicación, orquestando el flujo del caso de uso.
    - RA4: No importa nada de Flask, HTTP, request, ni CSV.
    - RA5 (DIP): Depende de la ABSTRACCIÓN RangeRepository declarada en el dominio
      (backend.domain.ports.range_repository), nunca de una implementación concreta.
    - RA6: Recibe objetos de dominio ya construidos y validados (Plant, Measurement).
      Nunca recibe requests HTTP, dicts crudos ni MultiDicts de Werkzeug.
    - SRP: Su única responsabilidad es coordinar el diagnóstico de la planta.
    """
    ...
    def diagnose(self, plant: Union[Plant, str], measurement: Measurement) -> dict:
        ...
        :param plant: Entidad Plant o string con la especie.
        :param measurement: Value object Measurement validado.
        :return: Diccionario estructurado con el estado, niveles de parámetros y recomendaciones (RF1-RF4).
        """
    ...
    def _determine_status(levels: Dict[str, IndicatorLevel]) -> PlantStatus:
        """
        Deriva el estado global de la planta según RF3.
        - SALUDABLE: Todos los indicadores están en nivel OPTIMO.
        - CRITICO: Presencia de carencia severa (BAJO) o múltiples parámetros fuera de rango.
        - EN_RIESGO: Desviación moderada.
        """
    ...
    def _generate_recommendations(levels: Dict[str, IndicatorLevel], ranges: dict) -> List[str]:
        """
        Genera recomendaciones textuales para cada parámetro fuera de rango (RF4).
        """
        recs: List[str] = []
